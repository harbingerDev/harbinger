...
async function createProjectOnDisk(req) {
  try {
    ...
    await new Promise((resolve, reject) => {
      ...
      exec(
        "npm init -y && npm i -D @faker-js/faker @ajv @playwright/test @reportportal/agent-js-playwright dotenv && npx playwright install && git init && git remote add origin git@github.com:codetesta2z/kilimanjaro.git",
        (error, stdout) => {
          if (error) {
